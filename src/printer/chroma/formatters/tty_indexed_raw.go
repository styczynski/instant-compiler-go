...
var ttyRawTables = map[int]*ttyTable{
	8: {
...
func rawEntryToEscapeSequence(table *ttyTable, entry chroma.StyleEntry) string {
	out := ""
...
func rawStyleToEscapeSequence(table *ttyTable, style *chroma.Style) map[chroma.TokenType]string {
	style = clearBackground(style)
	out := map[chroma.TokenType]string{}
	for _, ttype := range style.Types() {
		entry := style.Get(ttype)
		out[ttype] = rawEntryToEscapeSequence(table, entry)
	}
	return out
}

type indexedRawTTYFormatter struct {
	table *ttyTable
}

func (c *indexedRawTTYFormatter) Format(w io.Writer, style *chroma.Style, it chroma.Iterator) (err error) {
	theme := rawStyleToEscapeSequence(c.table, style)
...
var TTY16Raw = Register("terminal16raw", &indexedTTYFormatter{ttyRawTables[16]})
