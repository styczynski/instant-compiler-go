...
func (CompilerPipelineFactory) Params(argSpec *config.EntityArgSpec) {
	argSpec.AddString("profile", "", "Specify output file for profiling")
...
func (CompilerPipeline) RunPipeline(c config.EntityConfig, reader input_reader.InputReader) (string, []runner.LatteRunnedProgram, bool) {
	logs.InitializeLogger(c.String("log-level"), c.String("log-output"))

	profileFile := c.String("profile")
	if len(profileFile) > 0 {
		f, _ := os.Create(profileFile)
	...
	logs.UseParsingContext(context)
