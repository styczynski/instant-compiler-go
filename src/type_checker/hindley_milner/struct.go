...
func (t *SignedStruct) Union(other interface{}, context Constraint, infer InferenceBackend, listener IntrospecionListener) (Subs, error) {
	if ot, ok := other.(*SignedStruct); ok {
		...
		for i, t1 := range t.ts {
			if t2, ok := ot.ts[i]; ok {
				subs, err := Unify(t1, t2, context, infer, listener)
				if err != nil {
					return nil, err
				}
